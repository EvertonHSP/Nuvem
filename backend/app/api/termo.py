    ...
    def post(self):
        ...
        if args['aceito']:
            ...
            return {
                ...
                "data_aceite": data_aceite.isoformat()  # Garantido que não é None
            }, 200
        else:
            ...
            return {
                ...
                "data_exclusao": data_exclusao.isoformat(),  # Garantido que não é None
...
def check_terms_version():
    ...
    try:
        # Obtém o caminho absoluto para o arquivo de termos
        terms_file_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)),  # Volta para app/
            'terms',
            'termos_uso.html'
        )
        ...
        # Obtém a versão atual ativa no banco de dados
        current_terms = PoliticaSistema.query.filter_by(
            tipo_politica='uso',
            ativa=True
        ).first()
        
        # Se existirem termos ativos e o conteúdo for diferente
        if current_terms and current_terms.conteudo_termos != file_content:
            # Gera uma nova versão (8 primeiros caracteres de um UUID)
            new_version = str(uuid4())[:8]
            
            # Desativa a versão anterior
            current_terms.ativa = False
            
            # Cria novos termos com o conteúdo atualizado
